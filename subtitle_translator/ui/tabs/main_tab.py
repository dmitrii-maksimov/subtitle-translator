...
def build_main_tab(window, parent):
    ...
    window.btn_live = QPushButton("File downloading (live)…")
    window.btn_live.clicked.connect(window.on_open_live_dialog)
    window.btn_kodi_follow = QPushButton("Following Kodi…")
    window.btn_kodi_follow.clicked.connect(window.on_open_kodi_follow_dialog)
    file_layout.addWidget(window.file_label, 1)
    file_layout.addWidget(btn_browse)
    file_layout.addWidget(btn_browse_folder)
    file_layout.addWidget(window.btn_live)
    file_layout.addWidget(window.btn_kodi_follow)
